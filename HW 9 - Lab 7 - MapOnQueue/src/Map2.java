...
public class Map2<K, V> extends MapSecondary<K, V> {
    ...
    private static <K, V> void moveToFront(Queue<Pair<K, V>> q, K key) {
        ...
        Queue<Pair<K, V>> keyQueue = q.newInstance();
        Queue<Pair<K, V>> restQueue = q.newInstance();
        for (Pair<K, V> pair : q) {
            if (pair.key().equals(key)) {
                keyQueue.enqueue(pair);
            } else {
                restQueue.enqueue(pair);
            }
        }
        keyQueue.append(restQueue);
        q.transferFrom(keyQueue);
    }
    ...
    public final void add(K key, V value) {
        ...
        this.pairsQueue.enqueue(new SimplePair<K, V>(key, value));
    }
