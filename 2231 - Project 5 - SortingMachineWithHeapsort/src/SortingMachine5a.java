...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
     ...
     */
    private static <T> void exchangeEntries(Array<T> array, int i, int j) {
        T temp = array.entry(i);
        array.setEntry(i, array.entry(j));
        array.setEntry(j, temp);
    }
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (left <= last) {
            if (right <= last) {
                if (order.compare(array.entry(right), array.entry(left)) < 0) {
                    smaller = right;
                } else {
                    smaller = left;
                }
                if (order.compare(array.entry(top), array.entry(smaller)) > 0) {
    ...
    public final T removeFirst() {
        ...
        T removed = this.heap.entry(this.heapSize - 1);
        this.heapSize--;
    ...
    private final class SortingMachine5aIterator implements Iterator<T> {
        ...
        public T next() {
            ...
            if (SortingMachine5a.this.insertionMode) {
                ...
                next = SortingMachine5a.this.heap.entry(this.arrayCurrentIndex);
