...
public class Set2<T> extends SetSecondary<T> {
    ...
    private static <T> void moveToFront(Queue<T> q, T x) {
        ...
        Queue<T> xQueue = q.newInstance();
        Queue<T> restQueue = q.newInstance();
        for (T element : q) {
            if (element.equals(x)) {
                xQueue.enqueue(element);
            } else {
                restQueue.enqueue(element);
            }
        }
        xQueue.append(restQueue);
        q.transferFrom(xQueue);
    }
